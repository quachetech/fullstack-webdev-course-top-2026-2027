# 190 Days of The Odin Project Foundations

**Challenge Start Date:** 28 March 2026  
**Challenge End Date:** 4 October 2026  
**Current Streak:** 🔥 4 days

---

## 🎯 Mission

Master the fundamentals of web development through The Odin Project's Foundations course.

This time, I'm not rushing. I'm building **deep understanding** through:
- Daily practice (minimum 15 minutes/day)
- Feynman Technique explanations (recorded on YouTube)
- Detailed documentation (committed to GitHub)

**Why I'm doing this:**  
To build the technical foundation for my Biorhythmic Productivity Systems (BPS) research and SaaS platform.

---

## 📊 Progress Tracker

| Day | Date     | Topic           | Concept Explained | Project Worked On | Status |
|-----|----------|-----------------|-------------------|-------------------|--------|
| 000 | 28/03/26 | Challenge Launch & Set Up| ✅ Current Page   | - | ✅ |
| 001 | 29/03/26 | Introduction to Web Development | ✅ (./concepts/introduction-to-web-development.md) | - | ✅ |
| 002 | 30/03/26 | Asking For Help    | ✅(./concepts/asking-for-help.md) | - | ✅ |
| 003 | 31/03/26 | How The Internet Works | ✅(./concepts/how-the-internet-works.md)  | - | ✅ |
| 004 | 01/04/26 | Browsing The Web | ✅(./concepts/how-the-internet-works.md) | - | 🚧 |
| xxx | ... | ... | ... | ... | ... |
| xxx | ... | ... | ... | ... | ... |

**Legend:**
- ✅ Completed
- 🚧 In Progress
- ⏸️ Paused
- 📌 Planned             

---

## 🧠 Key Concepts Mastered

- [Introduction to Web Development](./concepts/introduction-to-web-development.md)
- [Asking For Help](./concepts/asking-for-help.md)
- [How The Interet Works](./concepts/how-the-internet-works.md)
- [Browsing The Web](./concepts/browsing-the-web.md)
- [JavaScript Functions](./concepts/javascript-functions.md)  
- [...](./concepts/xxx.md)

---

## 🛠️ Projects Built

1. **[Landing Page](./projects/01-landing-page/)** - HTML/CSS fundamentals
2. **[Rock Paper Scissors](./projects/02-rock-paper-scissors/)** - JavaScript logic
3. **[Etch-a-Sketch](./projects/03-etch-a-sketch/)** - DOM manipulation
4. *(These are placeholders to be updated once projects are available)*

---

## 📹 YouTube Explanations

- [Day 1: Introduction to Web Development](https://youtu.be/ivUAwME_bN0)
- [Day 2: Asking For Help](https://youtu.be/PF6qai_blFk)
- [Day 3: How The Internet Works](https://youtu.be/m0UeSgzwFBg)
- [Day 4: Browsin The Web](https://youtu.be/Ea57yNQ6F9s)
- [Day x: xxx](https://youtu.be/)
---

## 📝 Daily Log

See [daily-log/](./daily-log/) folder for day-by-day updates.

---

## 🔗 Connect With Me

- **LinkedIn:** [Kuda M'ndau](https://www.linkedin.com/in/kuda-mndau/)
- **YouTube:** [Elevate & Excel](https://www.youtube.com/@elevatenexcelHP)

---

## 💡 Why I Restarted

I completed ~80% of The Odin Project previously, but I was rushing. This time:
- **Depth over speed**
- **Understanding over completion**
- **Mastery over checking boxes**

Follow my journey: **#190DaysTOPFoundations**

---

## 📌 Challenge Rules

1. **Daily commitment:** Minimum 15 minutes of learning or coding
2. **Document everything:** Commit to GitHub daily
3. **Teach to learn:** Record Feynman-style explanations
4. **Build, don't just consume:** Complete all projects
5. **No tutorial hell:** Struggle first, then seek help

---

**Let's build. 🚀**